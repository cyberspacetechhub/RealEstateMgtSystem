...
import CreateOwnersModal from './CreateOwnersModal';
...
const AdminOwners = () => {
  ...
  return (
    <div className=' pt-5 max-sm:mt-10 h-screen'>
          ...
          <table className="w-full min-w-max border-collapse bg-white text-left text-sm text-gray-500 max-lg:w-full">
            ...
            <tbody className="divide-y divide-gray-100 border-t border-gray-100">
              { Array.isArray(data?.owners) && data?.owners.length > 0 ?(
                ...
                <th className="flex gap-3 px-6 py-4 font-normal text-gray-900">
                  <div className="relative h-10 w-10">
                    <img
                      ...
                      alt="Profile"
                    />
                    <span className={`absolute right-0 bottom-0 h-2 w-2 rounded-full ${owner.status === 'Active' ? 'bg-green-400' : 'bg-red-600'} ring ring-white`}></span>
                  </div>
                  <div className="text-sm">
                    <div className="font-medium text-gray-700 text-start">{`${owner.firstname} ${owner.lastname}`}</div>
                    <div className="text-gray-400">{owner.email}</div>
                  </div>
                </th>
                <td className="px-6 py-4">
                  <span className={`inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-1 text-xs font-semibold ${owner.status === 'Active' ? 'text-green-600' : 'text-red-600 bg-red-50'}`}>
                    <span className={`${owner.status === 'Active' ? 'h-1.5 w-1.5 rounded-full bg-green-600' : 'h-1.5 w-1.5 rounded-full bg-red-600'}`}></span>
                    {owner.status}
                  </span>
                </td>
                <td className="px-6 py-4">{owner.createdAt.substring(0,10)}</td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="">
                    <button onClick={() => handleTools(owner._id)}>
                    <svg 
                    ...
                    <Link
                    to={`/admin/owner_details/${owner._id}`}
                      title='view'
      ...
      <CreateOwnersModal open={openModal} handleClose={handleClose} />
